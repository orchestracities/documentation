...
export class CrateDatasource {
  ...
  annotationQuery(options) {
    ...
    return this._post('_sql', data).then(data => {
      if (!data || !data.rows || !data.rows[0]) {
        throw { message: 'No results' };
      }
      return this.transformAnnotationResponse(options, data);
    });
  }
  ...
  transformAnnotationResponse(options, data){
    ...
    _.each(data.cols, (column, index) => {
      ...
      if (_.includes((options.annotation.tagsColumn || '').replace(' ', '').split(','), column)) {
        tagsCol.push(index);
        return;
      }
