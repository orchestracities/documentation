    ...
    return {
        ...
        execute: function() {
            CrateDatasource = (function () {
                ...
                CrateDatasource.prototype.annotationQuery = function (options) {
                    var _this = this;
                    ...
                    return this._post('_sql', data).then(function (data) {
                        if (!data || !data.rows || !data.rows[0]) {
                            throw { message: 'No results' };
                        }
                        return _this.transformAnnotationResponse(options, data);
                    });
                };
                CrateDatasource.prototype.transformAnnotationResponse = function (options, data) {
                    var list = [];
                    var titleCol = null;
                    var timeCol = null;
                    var tagsCol = [];
                    var textCol = null;
                    lodash_1["default"].each(data.cols, function (column, index) {
                        if (column === 'time') {
                            timeCol = index;
                            return;
                        }
                        ...
                        if (column === options.annotation.titleColumn) {
                            titleCol = index;
                            return;
                        }
                        if (lodash_1["default"].includes((options.annotation.tagsColumn || '').replace(' ', '').split(','), column)) {
                            tagsCol.push(index);
                            return;
                        }
                        if (column === options.annotation.textColumn) {
                            textCol = index;
                            return;
                        }
                    });
                    lodash_1["default"].each(data.rows, function (value) {
                        var data = {
